...
def bangumi_subject_parser(subject_page):
	...
	subject_data['cover'] = 'https:'+columnSubjectHomeA.div.a['href'] # 是否下载？TODO
	baseinfos = columnSubjectHomeA.ul.select('li')
	for baseinfo in baseinfos:
		# 清洗数据！TODO
		if not isinstance(baseinfo.contents[1], str):
			# 不是文本是超链接的情况
			subject_data['baseinfos'][baseinfo.contents[0].text[:-2]] = baseinfo.contents[1].text 
		else:
			# 去掉":"的干扰 然后按索引写字典
			subject_data['baseinfos'][baseinfo.contents[0].text[:-2]] = baseinfo.contents[1]
	...
	tags = columnSubjectHomeB.find('div','subject_tag_section').div.select('a')
	for tag in tags:
		# 清洗数据！TODO
		subject_data['tags']['label'][tag.span.text] = ('https://bgm.tv'+tag['href'], int(tag.small.text))
	subject_data['ranking']['bgmtv'] = float(columnSubjectHomeB.find('span','number').text)
